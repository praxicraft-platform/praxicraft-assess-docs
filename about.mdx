...
- **API key** — `ct_live_…` (live) or `ct_test_…` (test) Bearer credential (Starter+). Created under **Developer → API Keys** by owners, admins, or [developers](/roles). Empty scopes mean full access (legacy).
- **Hiring squad** — Growth+ sub-team used to filter assessments and pipelines.

## Where to start

- **Explore by use case** — [Technical assessments](/assessments), [Interviews & pipelines](/pipelines), [ATS & automations](/automations)
- **Team access** — [Team roles](/roles) (invite a **developer** for API keys without granting billing)
- **Account / API key** — [create a key](/authentication) (or stay in the [dashboard](https://assess.praxicraft.com/assess) with no key)
- **SDKs** — [Python, Node.js, Go](/sdks/introduction)
- **First invite** — [Quickstart](/quickstart)
- **AI-ready docs** — [Build with agents](/build-with-agents), [Using docs with LLMs](/using-llms), and [MCP](/assess-mcp)
